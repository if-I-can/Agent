...
DEFAULT_PROMPT = """
Convert the text recognized in the image to markdown format. Adhere strictly to these guidelines:

1. Output in the same language as detected in the image. For instance, if English text is recognized, the output must be in English.

2. Directly transcribe the image content without adding any explanations or extraneous text. Do not preface the output with phrases like "Here's the markdown text generated from the image content:". Simply begin with the markdown content.

3. Formatting instructions:
   - Do not enclose the entire content within ```markdown ``` blocks.
   - Use $$ $$ for block equations and $ $ for inline equations.
   - Ignore long horizontal lines and page numbers.

Remember: Focus solely on transcribing the image content. Do not add any commentary or explanations outside of what is visible in the image.
"""

DEFAULT_RECT_PROMPT = """
The image contains areas marked with red boxes and labels (%s). Process these areas as follows:

- For tables or images: Insert them using the markdown format ![]()
- For text: Directly transcribe the content

Maintain the structure and layout of the original document as closely as possible within the constraints of markdown formatting.
"""


DEFAULT_ROLE_PROMPT = """
You are a sophisticated PDF document parser. Your task is to accurately transcribe the content of images into markdown format, incorporating LaTeX syntax for mathematical expressions when necessary. Preserve the original document's structure and formatting to the best of your ability within the constraints of markdown and LaTeX.
"""
...
def _gpt_parse_images(
    ...
    **args,
    ...
    def _process_page(index: int, image_info: Tuple[str, List[str]]) -> Tuple[int, str]:
        ...
        agent = Agent(
            role=role_prompt, api_key=api_key, base_url=base_url, model=model, **args
        )
        page_image, rect_images = image_info
        local_prompt = prompt
        if rect_images:
            local_prompt += rect_prompt + ", ".join(rect_images)
        content = agent.run([local_prompt, {"image": page_image}], display=True)
...
def process_pdf(pdf_path, output_dir):
    ...
    load_dotenv()
    api_key = "sk-yqowtakcfanhiciuhkemuoinyhpfbykjuuqmpyplstgmzbfs"
    base_url = "https://api.siliconflow.cn/v1"
    model = "Qwen/Qwen2-VL-72B-Instruct"

    # 获取pdf文件名称，不带扩展名
    pdf_name = os.path.splitext(os.path.basename(pdf_path))[0]
    output_md = f"{pdf_name}.md"
    ...
    print(f"内容已保存到: {output_dir}/{output_md}")
    print(f"处理的图片数量: {len(image_paths)}")


if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("用法: python3 process_pdf.py <pdf文件路径> <输出目录>")
        sys.exit(1)
